...
def test_steady_Burgers_vms():
    ...
    p['pmain'] = 'TestCaseResults/' + p['Case']
...
def test_steady_ErikssonJhonsson():
    ...
    p.update(steady_updates)
    p['Linfx'], p['Linfy'], p['Lsupx'], p['Lsupy'] = 0, 0, 1, 1   
    p['pmain'] = 'TestCaseResults/' + p['Case'] #+ folderset
    p['mesh'] = MeshGenerator(p).Create_mesh()
    p['boundaries'] = BoundaryCreator(p,p['mesh']).create_boundaries()
...
class TestASFEMCases():
    ...
    def test_fichera(self):
        ...
        print("Fichera: Finished")

    def test_heterogeneous(self):
        result = test_steady_Heterogeneous_vms()
        print("Heterogeneous: Finished")
 
    def test_anisotropic(self):
        result = test_steady_Anisotropic_vms()
        print("Anisotropic: Finished")

    def test_eriksson(self):
        result = test_steady_ErikssonJhonsson()
        print("Eriksson: Finished")

    def test_unsteady_be(self):
        result = test_unsteady_ErikssonJhonssonBE()
        print("UnsteadyBE: Finished")

    def test_unsteady_bdf2(self):
        result = test_unsteady_ErikssonJhonssonBDF2()
        print("UnsteadyBDF2: Finished")

    def test_burgers(self):
        result = test_steady_Burgers_vms()
        print("Burgers: Finished")

if __name__ == '__main__':
    case = TestASFEMCases()
    test0 = case.test_poisson() #test: 1 RL
    test1 = case.test_eriksson() #test: 23 RL
    test2 = case.test_fichera()  #test: 8 RL
    test3 = case.test_heterogeneous()  #test: 9 RL
    test4 = case.test_anisotropic() #done 23 RL 
    test5 = case.test_unsteady_be() #done 23 RL - T:0.1 - dt:0.005
    test6 = case.test_unsteady_bdf2() #done 23 RL - T:0.1 - dt:0.005
    test7 = case.test_burgers() #done 40 RL
