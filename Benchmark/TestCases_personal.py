...
class TestASFEMCases():

    def test_poisson(self):
        result = test_poisson_FEM_solution()
        assert result == pytest.approx(0.2471405369044419, rel=1e-10)
        print("OK")

    def test_fichera(self):
        result = test_steady_Fichera_vms()
        assert result == pytest.approx(0.0036270308223143605, rel=1e-10)
        print("OK")

    def test_heterogeneous(self):
        result = test_steady_Heterogeneous_vms()
        assert result == pytest.approx(0.0025428740360954874, rel=1e-10)
        print("OK")
 
    def test_anisotropic(self):
        result = test_steady_Anisotropic_vms()
        assert result == pytest.approx(0.0004639546948881584, rel=1e-10)
        print("OK")

    def test_eriksson(self):
        result = test_steady_ErikssonJhonsson()
        assert result == pytest.approx(0.016434803708377115, rel=1e-10)
        print("OK")

    def test_unsteady_be(self):
        result = test_unsteady_ErikssonJhonssonBE()
        assert result == pytest.approx(0.09013095732710719, rel=1e-10)
        print("OK")

    def test_unsteady_bdf2(self):
        result = test_unsteady_ErikssonJhonssonBDF2()
        assert result == pytest.approx(0.059916400178857125, rel=1e-6)
        print("OK")

    def test_burgers(self):
        result = test_steady_Burgers_vms()
        assert result == pytest.approx(0.08007984881050467, rel=1e-10)
        print("OK")

if __name__ == '__main__':
    case = TestASFEMCases()
    test0 = case.test_poisson() 
    test1 = case.test_eriksson() 
    test2 = case.test_fichera() 
    test3 = case.test_heterogeneous() 
    test4 = case.test_anisotropic() 
    test5 = case.test_unsteady_be() 
    test6 = case.test_unsteady_bdf2()
    test7 = case.test_burgers() 
